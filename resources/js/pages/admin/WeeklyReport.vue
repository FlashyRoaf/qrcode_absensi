        ...
        <div class="min-h-screen bg-zinc-950 text-zinc-100 font-mono">
            ...
            <header class="border-b border-zinc-800 bg-zinc-950/90 backdrop-blur sticky top-0 z-40">
                <div class="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
                    ...
                    <div class="text-xs text-zinc-500">
                        Target: <span class="text-emerald-400 font-bold">14.5 jam / minggu</span>
                    </div>
                </div>
            </header>
            ...
            <main class="max-w-7xl mx-auto px-6 py-8 space-y-8">
                ...
                <section class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                    <div
                        class="col-span-2 md:col-span-1 lg:col-span-1 bg-zinc-900 border border-zinc-800 rounded-xl p-4 flex flex-col gap-1">
                    ...
                    <div class="bg-zinc-900 border border-zinc-800 rounded-xl p-4 flex flex-col gap-1">
                        ...
                        <p class="text-3xl font-bold"
                            :class="complianceRate >= 70 ? 'text-emerald-400' : 'text-red-400'">
                            {{ complianceRate }}%
                        </p>
                    </div>
                </section>
                ...
                <section class="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
                    <div class="flex items-center justify-between mb-3">
                        <p class="text-xs font-bold text-zinc-400 uppercase tracking-widest">Tingkat Kepatuhan
                            Keseluruhan</p>
                ...
                <section v-if="activeTab === 'tabel'" class="space-y-4">
                    ...
                    <div class="flex items-center gap-3 flex-wrap">
                        <span class="text-xs text-zinc-500 uppercase tracking-widest font-mono">Minggu:</span>

                        <div class="flex items-center gap-2 bg-zinc-900 border rounded-lg px-3 py-2 transition"
                            :class="selectedWeek ? 'border-emerald-500/50' : 'border-zinc-700'">
                            <svg class="w-3.5 h-3.5 text-zinc-500 flex-shrink-0" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                            </svg>
                            <input type="date" :value="selectedWeek" @change="handleWeekInput"
                                class="bg-transparent text-sm text-zinc-200 focus:outline-none w-36" />
                        </div>

                        <!-- Info snap ke Senin -->
                        <span v-if="selectedWeek" class="text-xs text-zinc-500 font-mono">
                            Minggu: <span class="text-emerald-400">{{ selectedWeek }}</span>
                            <button @click="clearWeek"
                                class="ml-2 text-zinc-600 hover:text-red-400 transition">✕</button>
                        </span>
                        <span v-else class="text-xs text-zinc-600 italic">Semua minggu</span>
                    </div>
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="bg-zinc-900 border-b border-zinc-800">
                                    ...
                                    <th class="px-4 py-3 text-left text-xs text-zinc-400 uppercase tracking-widest">
                                        Progress</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr v-for="(row, i) in paginated" :key="i"
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-2">
                                            <div
                                                class="w-7 h-7 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-xs font-bold text-zinc-400">
                                                {{ String(row.user_name || row.user_id).charAt(0).toUpperCase() }}
                                            </div>
                                            <div>
                                                <p class="text-zinc-200 font-semibold text-xs">{{ row.user_name || `User
                                                    #${row.user_id}` }}</p>
                                    ...
                                    <td class="px-4 py-3">
                                        <span class="font-bold"
                                            :class="row.total_minutes >= 870 ? 'text-emerald-400' : 'text-red-400'">
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-xs text-zinc-600">{{ Math.min(100,
                        ...
                        <div class="flex gap-1">
                            ...
                            <button v-for="p in totalPages" :key="p" @click="currentPage = p"
                                class="px-3 py-1.5 rounded-md border transition font-bold" :class="currentPage === p
                                    ? 'bg-emerald-500 border-emerald-500 text-black'
                                    : 'bg-zinc-900 border-zinc-700 text-zinc-400 hover:border-zinc-500'">{{ p
                                }}</button>
                            <button @click="currentPage = Math.min(totalPages, currentPage + 1)"
                                :disabled="currentPage === totalPages || totalPages === 0"
                                class="px-3 py-1.5 rounded-md bg-zinc-900 border border-zinc-700 hover:border-zinc-500 transition disabled:opacity-30">Next
                                →</button>
                        </div>
                    </div>
                </section>
                ...
                <section v-if="activeTab === 'user'" class="space-y-3">
                    ...
                    <div v-for="(u, i) in userStats" :key="u.user_id"
                        ...
                        <div class="flex items-center gap-4 flex-shrink-0">
                            ...
                            <div
                                class="w-10 h-10 rounded-full bg-zinc-800 border-2 border-zinc-700 flex items-center justify-center font-bold text-zinc-300">
                        ...
                        <div class="flex gap-6 flex-1 flex-wrap">
                            ...
                            <div class="text-center">
                                <p class="text-2xl font-bold text-zinc-400">{{ fmtMinutes(Math.round(u.total_minutes /
                                    u.total)) }}</p>
                        ...
                        <div class="flex-shrink-0 w-full md:w-40">
                            <div class="flex justify-between text-xs text-zinc-600 mb-1">
                                <span>Kepatuhan</span>
                                <span
                                    :class="Math.round((u.memenuhi / u.total) * 100) >= 70 ? 'text-emerald-400' : 'text-red-400'">
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="bg-zinc-900 border-b border-zinc-800">
                                    ...
                                    <th class="px-4 py-3 text-left text-xs text-zinc-400 uppercase tracking-widest">
                                        Total User</th>
                                    <th class="px-4 py-3 text-left text-xs text-emerald-600 uppercase tracking-widest">
                                        Memenuhi</th>
                                    <th class="px-4 py-3 text-left text-xs text-red-600 uppercase tracking-widest">Tidak
                                        Memenuhi</th>
                                    <th class="px-4 py-3 text-left text-xs text-zinc-400 uppercase tracking-widest">
                                        Kepatuhan</th>
                                </tr>
                                ...
                                <tr v-for="w in weeklyTrend" :key="w.week"
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-xs"
                                                :class="Math.round((w.memenuhi / w.total) * 100) >= 70 ? 'text-emerald-400' : 'text-red-400'">
