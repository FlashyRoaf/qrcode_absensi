  ...
  <AppLayout>
    <div class="page-root px-6 py-8 space-y-8">
        ...
        <div class="stat-card" style="--delay: 0.05s">
          ...
          <div>
            ...
            <p class="text-gray-900 dark:text-white font-semibold text-lg leading-tight">
              {{ stats.totalHours.toFixed(1) }}<span
                class="text-gray-400 dark:text-neutral-500 text-sm font-normal">h</span>
            </p>
          </div>
        </div>
      ...
      <div class="filter-enter flex flex-wrap items-center gap-4">
        <input v-model="search" type="text" placeholder="Cari nama user..."
          class="bg-transparent border border-gray-300 dark:border-zinc-700 rounded-lg px-4 py-2 text-sm text-gray-900 dark:text-white placeholder-gray-900 dark:placeholder-white focus:outline-none focus:border-emerald-500 transition"
           />
        ...
        <div class="filter-pill" :class="{ 'filter-pill--active': selectedDate }">
          ...
          <button v-if="selectedDate" @click="clearDate"
      ...
      <Transition name="hint">
        <div v-if="sortKey" class="sort-hint-bar">
          ...
          —
          <span class="text-gray-500 dark:text-neutral-400">{{ sortDir === 'asc' ? '↑ Ascending' : '↓ Descending'
            }}</span>
        </div>
      </Transition>
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <thead class="table-head">
              <tr>
                <th v-for="col in columns" :key="col.key" class="text-left py-4 px-6 text-sm font-medium">
                  <button @click="toggleSort(col.key)" class="sort-btn group"
                    :class="{ 'sort-btn--active': sortKey === col.key }" :title="col.hint">
                    <span>{{ col.label }}</span>
                    <span class="sort-icon-wrap">
                      <ArrowUp v-if="getSortIcon(col.key) === 'asc'" class="sort-arrow sort-arrow--active" />
                      <ArrowDown v-else-if="getSortIcon(col.key) === 'desc'" class="sort-arrow sort-arrow--active" />
                      <ArrowUpDown v-else class="sort-arrow sort-arrow--idle group-hover:opacity-50" />
                    </span>
                  </button>
                </th>
              </tr>
              ...
              <tr v-for="(record, index) in filteredRecords" :key="record.id" class="table-row"
                ...
                <td class="py-4 px-6">
                  <div class="hours-wrap">
                    ...
                    <div class="hours-bar" :style="{ '--w': Math.min(record.hoursWorked / 12, 1) }" />
                  </div>
                </td>
              </tr>
